...
class TaskInsertRun:
        ...
        def run(sub: str, path: str, **kwargs) -> None:
                ...
                wrap_cmd_call(
                        ['sbt', 'project importer', '-mem', '256','runMain valar.importer.DirectoryLoader "{}"'.format(path)],
                        cwd=ConfigUtils.gen_repo_path("valar")
                )
                TaskUtils.update_status('inserting sensors', ti.xcom_pull(task_ids='retrieve_sub_info'))
                wrap_cmd_call(
                        ['sbt', 'project importer', '-mem','1024', 'runMain valar.importer.SensorProcessor "{}"'.format(path)], cwd=ConfigUtils.gen_repo_path("valar")
                        )
