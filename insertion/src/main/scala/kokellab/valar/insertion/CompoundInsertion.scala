...
object CompoundInsertion {
	...
	private def insertCompound(data: CompoundData): (Option[CompoundsRow], Boolean) = {
		...
		compound map { c =>
			...
			//val chemspiderId = getChemspiderId(data)
	...
	private def getChemspiderId(data: CompoundData): Option[Int] = {
		Try(new Chemspider(ValarConfig.instance.chemspiderToken).fetchChemspiderIds(data.inchi)) map core.only
	} match {
		case Success(id: Int) => Some(id)
		case Failure(e) =>
			logger.warn(s"No ChemSpider IDs found for Inchi ${data.inchi}", e)
			None
	}
